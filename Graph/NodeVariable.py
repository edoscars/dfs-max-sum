...
class NodeVariable:
    ...
    def getVariableNeighbour(self):
        '''
            returns variable neighbours of the variable
        '''
        variable_neighbours=list()
        function_neighbours= self.getNeighbour()
        for f in function_neighbours:
            variables = f.getNeighbour()
            for v in variables:
                if(v.getId() != self.getId() ):
                    variable_neighbours.append(v)

        return variable_neighbours

    def getNeighbourNotBackedge(self):
        variable_neighbours = list()
        function_neighbours = self.getNeighbour()
        for f in function_neighbours:
            if f.getId() < 1000:
    ...
    def getNumberOfNeighbour(self):
        variable_neighbours = list()
        function_neighbours = list()

        for fun in self.getNeighbour():
            if fun.getId() < 1000:
                function_neighbours.append(fun)

        return len(function_neighbours)
