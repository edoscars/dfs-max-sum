...
class MailMan:
    ...
    def sendQMessage(self, x, f, value, iter):
        ...
        if iter == 0:
            ...
            '''
            retVal = not(self.equalsQ(self.qmessages[(x,f)], value))
    ...
    def sendRMessage(self, f, x, value, iter):
        ...
        if iter == 0:
            ...
            '''
                verify if the mapping has value as a result 
            '''
            retVal = not(self.equalsR(self.rmessages[(f,x)], value))
    ...
    def equalsR(self, message, mc):
        ...
        difference = self.difference(message,mc)
        if difference < self.epsilonR:
            return False
		
        self.epsilonR = difference / 4
            
        return True 
	
    def equalsQ(self, message, mc):
        for i in range(mc.size()):
            if(not(message.getValue(i) == mc.getValue(i))):
                return False
        ...
        if difference < self.epsilonQ:
            return False
            
        self.epsilonQ = difference / 4
